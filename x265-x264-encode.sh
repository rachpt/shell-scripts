...
do
	...
	videoname="-metadata title=${videoname%.*} -metadata comment=made_by_Linux_OS";
	if [ $videoencode == "x265" ]; then
	...
	nohup $ffmpegcode ${setpass}1 -an -f mp4 -y /dev/null && $ffmpegcode ${setpass}2 -c:a aac $audiorate -strict -2 ${FOLDER}/$myfolder/${tempname%.*}_${videoencode}_${out}.mp4 >/dev/null 2>&1
